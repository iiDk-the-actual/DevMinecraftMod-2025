...
// 2/14/2025
// holding up the code by hand !  ~ goldentrophy <3
...
namespace DevMinecraftMod.Scripts.Building
{
    public class MinecraftMod : MonoBehaviour
    {
        ...
        async void Start()
        {
            ...
            itemIndicator.transform.localScale = Vector3.one * 1.5f;
        ...
        private void Update()
        {
            ...
            */
            ...
            try
            {
                gripDown = ControllerInputPoller.instance.rightGrab;
                triggerDown = ControllerInputPoller.TriggerFloat(XRNode.RightHand) > 0.5f;

                if (IsSteam)
                {
                    p2Da = SteamVR_Actions.gorillaTag_LeftJoystick2DAxis.GetAxis(SteamVR_Input_Sources.LeftHand);
                    p2DaC = SteamVR_Actions.gorillaTag_RightJoystickClick.GetState(SteamVR_Input_Sources.RightHand);

                    p2DaC2 = SteamVR_Actions.gorillaTag_LeftJoystickClick.GetState(SteamVR_Input_Sources.LeftHand);
                }
                else
                {
                    ControllerInputPoller.instance.leftControllerDevice.TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxis, out p2Da);
                    ControllerInputPoller.instance.rightControllerDevice.TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxisClick, out p2DaC);

                    ControllerInputPoller.instance.leftControllerDevice.TryGetFeatureValue(UnityEngine.XR.CommonUsages.primary2DAxisClick, out p2DaC2);
                }
            }
            catch { }
